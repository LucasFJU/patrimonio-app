...
export function mergePluggyData(state,payload){
 ...
 for(const source of payload.accounts||[]){
  ...
  if(!account){
   ...
   const firstDate=(source.transactions||[]).map(t=>t.date).filter(d=>/^\d{4}-\d{2}-\d{2}$/.test(d)&&d>=state.startDate).sort()[0]||today();
  ...
  for(const row of source.transactions||[]){
   if(!row.id||known.has(row.id)||!/^\d{4}-\d{2}-\d{2}$/.test(row.date)||row.date<state.startDate||row.date>today()||!Number.isFinite(row.amount)||row.amount===0||row.status==='PENDING')continue;
