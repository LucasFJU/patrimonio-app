export const CATEGORIES = { cdb:'CDB', tesouro:'Tesouro Direto', acoes:'Ações brasileiras', internacional:'Internacional', caixa:'Caixa de dividendos' };
export const COLORS = {cdb:'#345f50',tesouro:'#8eb393',acoes:'#b9d866',internacional:'#8498c7',caixa:'#d4b16a'};
...
export function validateState(s){
 ...
 const keys=Object.keys(s.settings.allocation||{});if(keys.sort().join(',')!=='acoes,cdb,internacional,tesouro')throw new Error('Classes da distribuição inválidas.');
 const weights=Object.values(s.settings.allocation||{});if(weights.length!==4||weights.some(v=>!Number.isFinite(v)||v<0)||Math.abs(weights.reduce((a,b)=>a+b,0)-100)>.01)throw new Error('A distribuição deve somar 100%.');
 ...
 calculate(s,undefined,true);return s;
}
