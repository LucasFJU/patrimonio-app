...
export function rankStocks(stocks,now=new Date()){
 const eligible=[],excluded=[];for(const p of stocks){const a=assessStock(p,now);if(a.eligible)eligible.push(a.stock);else excluded.push({...p,reason:a.reason});}
 eligible.sort((a,b)=>b.score-a.score||a.ticker.localeCompare(b.ticker));
 const sectors={},topPicks=[];
 for(const p of eligible){if(topPicks.length<5&&(sectors[p.sector]||0)<2){topPicks.push(p.ticker);sectors[p.sector]=(sectors[p.sector]||0)+1;}}
 return {month:now.toLocaleDateString('en-CA',{timeZone:'America/Sao_Paulo',year:'numeric',month:'2-digit'}),generatedAt:now.toISOString(),methodologyVersion:'1.1',picks:eligible,topPicks,excluded,coverage:{total:stocks.length,eligible:eligible.length},limitations:['Universo limitado a 15 empresas; não são as melhores de toda a Bolsa.','Volume financeiro estimado por fechamento × quantidade negociada; não é o volume financeiro exato da B3.','Triagem quantitativa, sem leitura automática de notícias, auditoria ou adequação individual.']};
}
