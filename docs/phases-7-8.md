...
- Medição local: `node scripts/benchmark-history.mjs`, com 5.000 eventos e 81 competências, produziu exatamente a mesma série e passou de **99,7 ms** (implementação anterior) para **1,6 ms** (atual), uma melhoria de **60,6×** nesta amostra.
- O JavaScript inicial da rota passou de aproximadamente **428.900 bytes** (baseline: chunk da página de 414.523 bytes + framework de 14.377 bytes) para **355.483 bytes**, redução de **17,1%**. Controle financeiro, painel, carteira, relatórios e metas são carregados por demanda ao abrir a área correspondente.
- A verificação dos tokens encontrou contraste mínimo de 4,5:1 para textos principais/secundários nos temas claro e escuro; uma regressão automatizada agora protege esse mínimo. No controle financeiro, lançamentos móveis podem ser editados; compras parceladas recalculam faturas e aportes manuais mantêm o evento correspondente na carteira.
...
- `node --test --test-isolation=none tests/*.test.mjs`: 69 testes aprovados.
